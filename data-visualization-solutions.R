...
ggplot(data = penguins_by_species, mapping = aes(x = species, y = n)) +
...
ggplot(
  data = penguins_by_species,
  mapping = aes(x = species, y = n, fill = species, label = n)
) +
  geom_col() +
...
ggplot(
  ...
  geom_col() +
  geom_label(vjust = -0.5, color = "white") +
  scale_fill_viridis_d() +
  scale_y_continuous(limits = c(0, 160), breaks = c(0, 40, 80, 120, 160))
...
ggplot(
  ...
  scale_fill_viridis_d() +
  scale_y_continuous(limits = c(0, 160), breaks = c(0, 40, 80, 120, 160)) +
  labs(title = "Number of penguins by species", x = NULL, y = NULL)
...
ggplot(
  data = penguin_weight_by_species_and_sex,
  mapping = aes(x = species, y = mean_weight, fill = sex)
) +
  geom_col() +
  facet_grid(cols = vars(sex)) +
  theme_minimal() +
  labs(title = "Average penguin weight by sex and species", x = NULL, y = NULL)
...
ggsave(
  filename = "plots/penguins-by-weight-species.png",
  height = 10,
  width = 20,
  units = "cm",
  bg = "white",
