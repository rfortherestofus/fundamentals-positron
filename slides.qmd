...
rru_table <- function(df) {
  df |>
    gt() |>
    tab_style(
      style = cell_text(weight = "normal", size = px(30), color = "#404e6b"),
      locations = cells_column_labels()
    ) |>
    tab_style(
      style = cell_text(size = px(30), color = "#404e6b"),
      locations = cells_body()
    ) |>
    gt_theme_538() |>
    opt_table_font(
      font = list(google_font("Inter"), default_fonts()),
      weight = 400
    ) |>
    tab_options(
...
- How to check version of Positron
...
1. Update all packages using the Positron R Package Manager extension
...
1. Copy the data into the Positron project
...
#| output: true
#| eval: false
mean(penguins$body_mass_g[penguins$year == 2007], na.rm = TRUE)
```

- Starts with function, data frame in parentheses

- Uses dollar sign (`data$variable`) notation

- Uses brackets (`[data$variable == XXXX]`)
...
::: {.notes}
First, go to user settings > RStudio keybindings
:::

## Your Turn {.your-turn}

1. Enable RStudio keybindings in Positron settings
...
ggplot(
  ...
  mapping = aes(x = year, y = income_change, color = income_group)
) +
  geom_line()
```
...
1. Paste the code below to into your Quarto file, keeping the title, setting the author as your name, and leaving the output format as `docx` (Word).

1. Save your Quarto file as `penguins-report.qmd`.

## Your Turn {.your-turn}

4. Look through this Quarto file and make sure that you can identify the YAML, text sections, and code chunks.

5. Click the Preview button to render your document and open the file that gets produced (`penguins-report.docx`).
...
1. Go into the YAML and change the title to "My Super Fancy Penguins Report."
...
1. Working in your `penguins-report.qmd` file, before Introduction, add a first-level heading that says "Funders"

## Your Turn {.your-turn}

::: {.fragment .nonincremental}
2. Add this text below the Funders first-level heading: 
...
3. Render your report again and make sure you see the changes.
...
1. Use text buttons in Positron
...
1. Adjust your code chunk options so that you do not see your code, any messages, or any warnings in the output (i.e. you should just see the text and charts).
